...
- **Scenario pressure anchors (transport, GasLib-582+)**: optional enrichment via `enrich_scenario_with_balance_hub` and iterative refinement. Real gain on **intact nomination**: residual 5 → **2.045 m³/s** (v13–v17). v18 contract Q relaxation lowers effective residual (~2.0) but **breaks GasLib nomination** — compare `nomination_mass_balance` vs `mass_balance` in `compressor_diag` JSON. Over-anchoring degrades residuals.
- Disable automatic routing with `GAZFLOW_SKIP_CDF_ROUTING=1` (or `GAZFLOW_SKIP_CDF=1`).
- **Compressor map modes** (`GAZFLOW_COMPRESSOR_MAP_MODE`): `legacy`, `measurement`, `biquadratic`. In-Newton map recoupling (v17) and optional head Jacobian (v19, `GAZFLOW_NEWTON_COMPRESSOR_HEAD_JAC=1`, default off). See [GasLib-582 bench](../testing/gaslib-582-compressor-bench.md).
